...
def _make_help_call(target, esc, lspace, next_input=None):
    ...
    if next_input is None:
        return '%sget_ipython().run_line_magic(%s, %r)' % (lspace, method, target)
    else:
        return '%sget_ipython().set_next_input(%r);get_ipython().run_line_magic(%s, %r)' % \
...
def _tr_magic(line_info):
    "Translate lines escaped with: %"
    tpl = '%sget_ipython().run_line_magic(%s, %r)'
...
assign_magic_template = '%s = get_ipython().run_line_magic(%s, %r)'
@StatelessInputTransformer.wrap
