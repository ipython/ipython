...
python-profiler package from non-free.""")
    ...
    def run(self, parameter_s='', runner=None,
        ...
        *two* back slashes (e.g., '\\\\*') to suppress expansions.
        ...
        -u: Start running the file at the first change since the last
        run. This automatically sets the i option so that the file has
        access to the namespace generated by previous runs. This is
        useful for keeping the buffer updated with changes made when
        working from a file. If m is passed as an option, the u option
        will be ignored.
        ...
        if "u" in opts and not "m" in opts:
            # make sure i is set
            if not "i" in opts:
                opts[u"i"] = [""]
            # file that tracks history of what has been loaded
            tracking_file = ".ipython-track-%s" % filename
            if not tracking_file in self.shell.tempfiles:
                self.shell.tempfiles.append(tracking_file)
            # file to write updated content to run
            change_file = self.shell.mktempfile(prefix="ipython-track-content-")
            # get name of appropriate file to pass to run
            filename = astdiff.get_change_file(filename, tracking_file,
                                               change_file)
            if filename is None:
                print('No changes found in %s' % arg_lst[0])
                return
