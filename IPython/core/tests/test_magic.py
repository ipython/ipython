...
@unittest.skip('Not ported yet, uses nt.__file__')
...
class TestResetErrors(unittest.TestCase):
...
@dec.skipif(execution.profile is None, "Test skipped when execution.profile is None")
...
@dec.skipif(execution.profile is None, "Test skipped when exe.profile is None")
...
class CellMagicTestCase(unittest.TestCase):
...
def test_script_bg_out():
    ...
    assert ip.user_ns['output'].read() == b'hi\n'
...
def test_script_bg_err():
    ...
    assert ip.user_ns['error'].read() == b'hello\n'
