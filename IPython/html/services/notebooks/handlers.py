...
class NotebookHandler(IPythonHandler):
    ...
    def post(self, path='', name=None):
        ...
        if name is not None:
            raise web.HTTPError(
                400, "Only POST to directories. Use PUT for full names.")
        ...
        if model is not None:
            ...
            if copy_from:
                if model.get('content'):
                    raise web.HTTPError(
                        400, "Can't upload and copy at the same time.")
    ...
    def put(self, path='', name=None):
        ...
        if name is None:
            raise web.HTTPError(
                400, "Only PUT to full names. Use POST for directories.")
...
default_handlers = [
    (r"/api/notebooks%s/checkpoints" %
     notebook_path_regex, NotebookCheckpointsHandler),
