...
class Testrevealhelp(PreprocessorTestsBase):
    ...
    def build_notebook(self):
        ...
        cells=[nbformat.new_code_cell(input="", prompt_number=1, outputs=outputs),
               nbformat.new_text_cell('markdown', source="new slide", metadata=slide),
               nbformat.new_code_cell(input="", prompt_number=2, outputs=outputs),
               nbformat.new_text_cell('markdown', source="new fragment", metadata=fragment),
               nbformat.new_text_cell('markdown', source="notes", metadata=notes),
               nbformat.new_text_cell('markdown', source="with last fragment", metadata=continuation),
               nbformat.new_text_cell('markdown', source="new fragment", metadata=fragment),
               nbformat.new_text_cell('markdown', source="skipped", metadata=skip),
               nbformat.new_text_cell('markdown', source="new subslide", metadata=subslide),
               nbformat.new_text_cell('markdown', source="with last subslide", metadata=continuation),
               ]
...
class TestSlideTypes(Testrevealhelp):
    def create_cells(self, slide_type, pre_cell_slide_types=(), post_cell_slide_types=()):
        ...
        complete_cell_types = list(pre_cell_slide_types) + [slide_type] + list(post_cell_slide_types)
    ...
    def test_slide_type(self):
        #Because of the reveal.js interface, a new slide should always make a new subslide too.
        self.assert_preprocessed_meta('slide', [], [],
                                      opens=['slide', 'subslide'], closes=['slide', 'subslide'])
        #A slide followed by a subslide should create just one top level slide.
    ...
    def test_subslide_type(self):
        self.assert_preprocessed_meta('subslide', [], [],
                                      opens=['slide', 'subslide'], closes=['slide', 'subslide'])
        #A subslide followed by a subslide should create just one top level slide.
