...
from IPython.nbconvert.utils.pandocfilters import walk
...
def pandoc_filtered( source, fmt, to, rec_filter,encoding='utf-8'):
    """Convert an input string in format 'fmt' to pandoc JSON, then apply the
    recursive filter 'rec_filter' to the JSON and eventually convert to format
    'to'.
    ...
    to : string
      The name of the output format (html, etc.)
    rec_filter : a pandocfilters function, signature is 
        rec_filter(key, value, format, meta) and return pandoc JSON(also called AST)
    ...
    json_string = pandoc(source, fmt, 'json', encoding=encoding)
    if json_string == "":
        return ""
    else:
