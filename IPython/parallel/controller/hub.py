...
class Hub(SessionFactory):
    ...
    def save_queue_request(self, idents, msg):
        ...
        try:
            ...
            self.log.error("queue::client %r sent invalid message to %r: %r",
                           client_id, queue_id, msg, exc_info=True)
            return
        ...
        if content['status'] == 'ok':
            if heart in self.heartmonitor.hearts:
                ...
                dc = ioloop.DelayedCallback(
                    purge, self.registration_timeout, self.loop)
                dc.start()
                self.incoming_registrations[heart] = EngineConnector(
                    id=eid, uuid=uuid, stallback=dc)
        else:
            self.log.error(
    ...
    def unregister_engine(self, ident, msg):
        ...
        dc = ioloop.DelayedCallback(
            handleit, self.registration_timeout, self.loop)
        dc.start()
        ############## TODO: HANDLE IT ################
