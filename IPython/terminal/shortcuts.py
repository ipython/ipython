...
def create_ipython_shortcuts(shell):
    ...
    kb.add('enter', filter=(HasFocus(DEFAULT_BUFFER)
                            & ~HasSelection()
                            & insert_mode
                        ))(return_handler)
    ...
    kb.add('c-p', filter=(ViInsertMode() & HasFocus(DEFAULT_BUFFER))
                )(previous_history_or_previous_completion)

    kb.add('c-n', filter=(ViInsertMode() & HasFocus(DEFAULT_BUFFER))
                )(next_history_or_next_completion)

    kb.add('c-g', filter=(HasFocus(DEFAULT_BUFFER) & HasCompletions())
                )(dismiss_completion)

    kb.add('c-c', filter=HasFocus(DEFAULT_BUFFER))(reset_buffer)

    kb.add('c-c', filter=HasFocus(SEARCH_BUFFER))(reset_search_buffer)
    ...
    kb.add('c-o', filter=(HasFocus(DEFAULT_BUFFER)
                  & EmacsInsertMode()))(newline_autoindent_outer(shell.input_splitter))

    kb.add('f2', filter=HasFocus(DEFAULT_BUFFER))(open_input_in_editor)

    if shell.display_completions == 'readlinelike':
        kb.add('c-i', filter=(HasFocus(DEFAULT_BUFFER)
                              & ~HasSelection()
                              & insert_mode
                              & ~cursor_in_leading_ws
    ...
    if sys.platform == 'win32':
        kb.add('c-v', filter=(HasFocus(DEFAULT_BUFFER) & ~ViMode()))(win_paste)
