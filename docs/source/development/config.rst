...
back to :file:`~/.ipython` for consistency among systems. If IPython finds
the XDG location alongside :file:`~/.ipython`, it will issue a warning and
use :file:`~/.ipython`; if the XDG directory is the only one present, it
will keep using it.
...
:file:`ipython_config.py`. An extra configuration file can be loaded in
addition to the defaults with the ``--config`` command line flag.
...
.. versionadded:: 5.0
    ...
    ``'0'`` or ``'false'`` (setting it to ``'1'`` or ``'true'`` forces
    suppression of all configuration errors).
...
logs, startup files, and your IPython command history.
