...
class MPIRectPartitioner2D(RectPartitioner2D):
    ...
    def update_internal_boundary(self, solution_array):
        ...
        if lower_x_neigh > -1:
            if self.slice_copy:
                self.out_lower_buffers[0] = ascontiguousarray(
                    solution_array[1, :])
            else:
        ...
        if upper_x_neigh > -1:
            ...
            if self.slice_copy:
                ...
                for i in xrange(0, loc_ny + 1):
                    ...
                    self.out_upper_buffers[0][
                        i] = solution_array[loc_nx - 1, i]
        ...
        if lower_y_neigh > -1:
            if self.slice_copy:
                self.out_lower_buffers[1] = ascontiguousarray(
                    solution_array[:, 1])
            else:
        ...
        if upper_y_neigh > -1:
            ...
            if self.slice_copy:
                ...
                for i in xrange(0, loc_nx + 1):
                    ...
                    self.out_upper_buffers[1][
                        i] = solution_array[i, loc_ny - 1]
