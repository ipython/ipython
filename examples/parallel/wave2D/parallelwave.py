...
if __name__ == '__main__':
    ...
    assert partition[
        0] * partition[1] == num_procs, "can't map partition %s to %i engines" % (partition, num_procs)
    ...
    # execute some files so that the classes we need will be defined on the
    # engines:
    ...
    view.apply_sync(setup_partitioner, Reference(
        'com'), peers, Reference('my_id'), num_procs, grid, partition)
    time.sleep(1)
    ...
    if ns.scalar:
        ...
        view.apply_sync(setup_solver, I, f, c, bc, Lx, Ly, partitioner=Reference(
            'partitioner'), dt=0, implementation=impl)

        # run first with element-wise Python operations for each cell
        t0 = time.time()
        ar = view.apply_async(
            _solve, tstop, dt=0, verbose=True, final_test=final_test, user_action=user_action)
        if final_test:
