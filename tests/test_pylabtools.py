...
def shell_pylab_fixture(hmmax3):
    ...
    matplotlib.rcParams = _saved_rcParams.copy()
    matplotlib.rcParamsOrig = _saved_rcParamsOrig.copy()
    ...
    s = Shell()
    yield s
    if s.history_manager is not None:
        if s.history_manager.save_thread is not None:
            s.history_manager.save_thread.stop()
        s.history_manager.db.close()
    s.configurables = []
