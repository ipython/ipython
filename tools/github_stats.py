...
def issues_closed_since(period=timedelta(days=365), project="ipython/ipython", pulls=False):
    ...
    filtered = [
        i for i in allclosed if _parse_datetime(i['closed_at']) > since]
    if pulls:
        filtered = [
            i for i in filtered if _parse_datetime(i['merged_at']) > since]
...
if __name__ == "__main__":
    ...
    if tag:
        ...
        all_authors = check_output(author_cmd).decode(
