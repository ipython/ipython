...
class TestRun(object):
    ...
    def setup(self):
        ...
        for py in self.available_python_versions():
            check_call(
                ['virtualenv', '-p', py, '--system-site-packages', 'venv-%s' % py])
        ...
        self.master_sha = check_output(
            ['git', 'log', '-1', '--format=%h']).decode('ascii').strip()
        os.chdir(basedir)
    ...
    def markdown_format(self):
        def format_result(result):
            ...
            if result.missing_libraries:
                s += " (libraries not available: " + \
                    result.missing_libraries + ")"
            return s
    ...
    def print_results(self):
        ...
        for result in self.results:
            if result.passed:
                ...
                print(
                    "    Test log:", result.get('log_url') or result.log_file)
            if result.missing_libraries:
...
def run_tests(venv, extra_args):
    ...
    os.environ["PATH"] = os.path.join(
        basedir, venv, 'bin') + ':' + os.environ["PATH"]
    os.environ.pop("PYTHONPATH", None)

    # check that the right IPython is imported
    ipython_file = check_output(
        [py, '-c', 'import IPython; print (IPython.__file__)'])
...
def test_pr(num, post_results=True, extra_args=None):
    ...
    if post_results:
        ...
        post_script = os.path.join(
            os.path.dirname(sys.argv[0]), "post_pr_test.py")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description="Test an IPython pull request")
